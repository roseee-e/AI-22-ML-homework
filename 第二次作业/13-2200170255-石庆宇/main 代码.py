...
def computeCost(X, Y, W,reg):
    ...
    l2_regularization = (reg/2)* np.sum(W1** 2)
    loss=loss+l2_regularization
    return loss
def gradientDescent(X, Y, W, alpha, reg):
    num_train = X.shape[0]
    Y_hat = np.dot(X, W)
    dW = X.T @ (Y_hat - Y) / num_train  # 计算原始梯度
    # 提取权重矩阵W中除了偏置项以外的部分
    W_reg = W[:-1]
    # 计算L2正则化项的梯度（不包括偏置项）
    dW_reg = reg * W_reg
    # 将正则化项的梯度加到除了偏置项以外的原始梯度中
    dW_with_reg = np.concatenate((dW_reg, [dW[-1]]), axis=0)  # 保持偏置项的梯度不变
    W -= alpha * dW_with_reg  # 使用梯度下降更新参数
    return W
...
ax.set_title('引入L2后的误差和训练Epoch数')
plt.show()
