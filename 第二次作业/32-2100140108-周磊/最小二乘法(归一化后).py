...
# 绘制训练和测试损失曲线
ax1.plot(range(num_iterations), train_losses, label='训练损失')
ax1.plot(range(num_iterations), test_losses, label='测试损失')
ax1.set_xlabel('迭代次数')
ax1.set_ylabel('损失')
ax1.legend()
...
ax2.scatter(x_test[:, 1], y_test, label='测试数据')
ax2.legend(loc=2)
ax2.set_xlabel('人口')
ax2.set_ylabel('收益', rotation=90)
ax2.set_title('预测收益和人口规模（最小二乘法）')
