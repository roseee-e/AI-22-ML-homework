        ...
        for label in class_scores:
            for i, feature_value in enumerate(featureset):
                if feature_value in self.features_prob[i]:
                    class_scores[label] += np.log(self.features_prob[i][feature_value].get(label, self.laplace / (self.class_counts[label] + self.laplace * len(self.feature_counts[i]))))
                else:
                    class_scores[label] += np.log(self.laplace / (self.class_counts[label] + self.laplace * len(self.feature_counts[i])))
